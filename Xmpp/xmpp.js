...
import "../services/ClientManager.js"
import User from "../src/db/models/User"
import Friends from "../src/db/models/Friends"
...
setInterval(() => {
  ...
  global.Clients.forEach((client, index) => {
    const lastHeartbeat = clientHeartbeats.get(client.accountId);
    
    if (lastHeartbeat && (now - lastHeartbeat) > CONNECTION_TIMEOUT) {
      try {
        client.client.terminate();
      } catch (e) {
        // Connection already closed
      }
      RemoveClient(client.client, client.joinedMUCs || []);
    }
  });
      ...
      try {
        client.client.send(
          XMLBuilder.create("iq")
            ...
            .attribute("id", `ping_${MakeID()}`)
            .attribute("type", "get")
            .attribute("xmlns", "jabber:client")
            .element("ping")
            .attribute("xmlns", "urn:xmpp:ping")
            .up()
            .toString()
        );
      } catch (error) {
        console.log(`Failed to send ping to ${client.displayName}: ${error.message}`);
      }
    }
  });
}, HEARTBEAT_INTERVAL);
...
app.get("/clients", (req, res) => {
  ...
  let data = JSON.stringify(
    {
      amount: global.Clients.length,
      clients: global.Clients.map(i => ({
        displayName: i.displayName,
        activity: i.lastPresenceUpdate?.status || "unknown"
      }))
    },
    null,
    2
...
app.get("/status", (req, res) => {
  ...
  const stats = {
    server: {
      ...
      connections: global.Clients.length
    },
    clients: global.Clients.map(client => ({
      displayName: client.displayName,
      accountId: client.accountId,
      jid: client.jid,
      lastHeartbeat: clientHeartbeats.get(client.accountId),
      timeSinceHeartbeat: now - (clientHeartbeats.get(client.accountId) || 0),
      connectionState: client.client.readyState,
      joinedMUCs: client.joinedMUCs ? client.joinedMUCs.length : 0
    })),
    mucs: Object.keys(global.MUCs).map(mucName => ({
      name: mucName,
      members: global.MUCs[mucName].members.length
    })),
    heartbeat: {
      interval: HEARTBEAT_INTERVAL,
      timeout: CONNECTION_TIMEOUT,
      activeHeartbeats: clientHeartbeats.size
    }
  }
...
app.get("/health", (req, res) => {
  ...
  const healthStatus = {
    ...
    clients: global.Clients.length,
    uptime: process.uptime()
  }
  
  res.send(JSON.stringify(healthStatus, null, 2))
})

wss.on("listening", () => {
  console.log(`XMPP started listening on port ${port}`)
})
...
wss.on("connection", async (ws, req) => {
  ...
  const initHeartbeat = (accountId) => {
    clientHeartbeats.set(accountId, Date.now());
  };

  const updateHeartbeat = (accountId) => {
    clientHeartbeats.set(accountId, Date.now());
  };
    ...
    try {
      ...
      switch (msg.root.name) {
        ...
        case "auth":
          ...
          if (
            ...
            Authenticated = true
            initHeartbeat(accountId);
            logger.login(`${displayName}`)
        ...
        case "iq":
          ...
          if (msg.root.attributes.type == "result" && 
              msg.root.attributes.id && 
              msg.root.attributes.id.startsWith("ping_")) {
            // Ping response received, update heartbeat
            if (accountId) {
              updateHeartbeat(accountId);
            }
            break;
        ...
        case "message":
          ...
          switch (msg.root.attributes.type) {
            case "chat":
              ...
              if (receiver.client.readyState === 1) { // WebSocket.OPEN
            ...
            default:
              if (
                ...
                global.MUCs[roomName].members.forEach(member => {
                  ...
                  if (ClientData.client.readyState === 1) { // WebSocket.OPEN
      ...
      if (!clientExists && !connectionClosed) {
        if (
          ...
          clientExists = true
          if (global.onClientConnect) global.onClientConnect(accountId);
        }
      }
    } catch (error) {
      console.log(`Error processing message from ${displayName || 'unknown'}: ${error.message}`);
      // Don't immediately close connection on error, just log it
    }
  })
  ...
  ws.on("close", (code, reason) => {
    ...
    clientExists = false
    if (accountId) {
      clientHeartbeats.delete(accountId);
    }
    RemoveClient(ws, joinedMUCs)
  })

  // Handle pong frames (WebSocket level heartbeat)
  ws.on("pong", () => {
    if (accountId) {
      updateHeartbeat(accountId);
    }
...
function Error(ws) {
  try {
    ...
    // Connection already closed or error sending
    try {
      ws.close()
    } catch (e) {
      // Ignore
...
function RemoveClient(ws, joinedMUCs) {
  ...
  if (partyId && typeof partyId == "string") {
    global.Clients.forEach(ClientData => {
      ...
      if (ClientData.client.readyState !== 1) return // WebSocket.OPEN
      ...
      try {
        ...
        console.log(`Failed to send party exit message: ${error.message}`);
      }
    })
  }

  if (global.onClientDisconnect) global.onClientDisconnect(client.accountId);

  logger.logout(`${client.displayName}`)
}
  ...
  try {
    ...
    accepted.forEach(friend => {
      ...
      try {
        ...
        console.log(`Failed to send presence to ${accountId}: ${error.message}`);
      }
    })
  } catch (error) {
    console.log(`Error getting presence from friends: ${error.message}`);
  }
}
  ...
  try {
    ...
    accepted.forEach(friend => {
      ...
      try {
        ...
        console.log(`Failed to send presence update: ${error.message}`);
      }
    })
  } catch (error) {
    console.log(`Error updating presence for friends: ${error.message}`);
  }
}
...
function sendXmppMessageToClient(senderJid, msg, body) {
  try {
    ...
    if (!receiver) return
    if (receiver.client.readyState !== 1) return // WebSocket.OPEN
    ...
    console.log(`Error sending XMPP message: ${error.message}`);
  }
}
